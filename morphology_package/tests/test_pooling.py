...
from morphological_torch.pooling_operations import MaxPool2D, ParameterizedMaxPool2D
from morphological_torch.baselines import BaselinePool2D, BaselineParameterizedPool2D
...
def _maxpool_test(dims, random=True, verbose=False, epsilon=1e-3):
    ...
    cuda_maxpool = MaxPool2D(k, stride=s).to(device)
...
def _parameterized_maxpool_test(dims, random=True, verbose=False, epsilon=1e-3):
    ...
    cuda_pool = ParameterizedMaxPool2D(c, k, s).to(device)
