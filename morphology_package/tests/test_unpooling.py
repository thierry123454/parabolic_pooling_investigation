...
from morphological_torch.pooling_operations import MaxPool2D
from morphological_torch.unpooling_operations import SparseUnpool2D, MaxUnpool2D, ParameterizedMaxUnpool2D
from morphological_torch.baselines import BaselinePool2D, BaselineMaxUnpool2D, BaselineUnpool2D, \
    BaselineParameterizedUnpool2D
...
def _unpool_test(dims, random=True, verbose=False, epsilon=1e-4):
    ...
    cuda_pool = MaxPool2D(k_in, stride=s).to(device)
    ...
    cuda_unpool = SparseUnpool2D(k_in, stride=s).to(device)
...
def _morph_unpool_test(dims, random=True, verbose=False, epsilon=1e-4):
    ...
    cuda_downsampled, cuda_indices_downsampled = cuda_pool(cuda_inputs)
    # Declare the two operations we are interested in.
    torch_unpool = BaselineUnpool2D(k_in, stride=s).to(device)
    cuda_unpool = MaxUnpool2D(k_in, times='single', stride=s).to(device)
    # Perform forward operations.
    t1 = _sync_and_time()
    torch_out = torch_unpool(torch_downsampled, torch_indices_downsampled, size=(h, w))
    t2 = _sync_and_time()
    cuda_out = cuda_unpool(cuda_downsampled, cuda_indices_downsampled, (h, w))
    t3 = _sync_and_time()
    cuda_out = torch.where(cuda_out == torch.tensor([-100.], dtype=torch.float32).cuda(),
                           torch.tensor([0.], dtype=torch.float32).cuda(), cuda_out)
    forward_success = (torch.abs(torch_out - cuda_out) < epsilon).all()
    # Now perform the backward operations.
    torch_loss = (torch_out * deltas).sum()
    t4 = _sync_and_time()
...
def _parameterized_morph_unpool_test(dims, random=True, verbose=False, epsilon=1e-4):
    ...
    # print(torch_downsampled)
    # print(cuda_downsampled)

    # Declare the two operations we are interested in.
    torch_unpool = BaselineParameterizedUnpool2D(c, k_in, stride=s).to(device)
    torch_unpool.h.data = se.view(1, c, k_out ** 2, 1)
    cuda_unpool = ParameterizedMaxUnpool2D(c, k_in, times='single', stride=s).to(device)
...
if __name__ == '__main__':
    ...
    _parameterized_morph_unpool_test((2, 8, 64, 64, 3, 2), random=True, verbose=True)
