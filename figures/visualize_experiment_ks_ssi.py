...
with open('experiments/kernel_size_experiment_ssi.json') as f:
    data = json.load(f)
...
fig.legend()
fig.suptitle("Scales learned for different window sizes using scale space initialization.", fontsize=30)
fig.tight_layout()
plt.subplots_adjust(top=0.9, bottom=0.07)
plt.savefig("figures/kernel_size_experiment_scales_ssi.pdf", format="pdf", bbox_inches="tight")
plt.show()
...
plt.legend()
plt.title("Accuracy and precision versus the kernel size of the parabolic structuring element using SSI.", fontdict={'fontsize': 15})
...
plt.savefig("figures/kernel_size_experiment_accuracy_and_precision_ssi.pdf", format="pdf", bbox_inches="tight")
...
plt.title("Time taken to train in seconds versus the kernel size of the standard parabolic structuring element using SSI.", fontdict={"fontsize": 15})
...
plt.savefig("figures/kernel_size_experiment_time_ssi.pdf", format="pdf", bbox_inches="tight")
