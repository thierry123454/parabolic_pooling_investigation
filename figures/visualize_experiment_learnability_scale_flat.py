...
with open('experiments/scale_learnability_experiment_flat.json') as f:
    data = json.load(f)
...
fig.suptitle("Scales learned on different epochs with varying starting scales using the approximation of a flat SE.", fontsize=22)
fig.tight_layout()
plt.subplots_adjust(top=0.9, bottom=0.07)
plt.savefig("figures/learnability_experiment_flat.pdf", format="pdf", bbox_inches="tight")
plt.show()
